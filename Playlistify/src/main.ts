...
async function start(): Promise<void> {
  ...
  // Prefer environment variables if they exist, otherwise use input, otherwise use default hardcoded values
  const clientId = process.env.SPOTIFY_CLIENT_ID || input?.spotifyClientId || 'f6b396ecab7646afab201c9eecaa7dd3';
  const clientSecret =
    process.env.SPOTIFY_CLIENT_SECRET || input?.spotifyClientSecret || 'fd407d0f8a0c49eebb0591ee77139544';
  const refreshToken =
    process.env.SPOTIFY_REFRESH_TOKEN || input?.spotifyRefreshToken || 'AQDs2gFJ-PcVZtSriscGAJuSQq34UMO8IHagDrToHQW1JnKKkayj8vyTj2iExt2M2ZjkKx9mXHYR9YZUK-f-W6kGWSEVEBebm17TwC7VXSHNf5CjYTbICCjrfioHvwBSSlc';
